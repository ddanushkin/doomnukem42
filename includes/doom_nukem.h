...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/03/23 17:35:08 by lglover           #+#    #+#             */
/*   Updated: 2020/03/23 18:20:04 by lglover          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_wall			*decor_add(t_v3d lp,
					t_sector *cs, t_wall *hit_w, t_camera *cam);
...
int				hit_fill_floor(t_app *app,
						t_v3d pos, t_v3d dir, double distance);
int				hit_fill_ceil(t_app *app,
						t_v3d pos, t_v3d dir, double distance);
void			player_crouch(t_app *app,
							t_camera *c, const uint8_t *key, double dt);
void			floor_collision(t_app *app, t_camera *c, double dt);
void			player_fly(t_app *app, t_camera *c);
void			player_jump(t_app *app);
void			player_movement(t_app *app,
							t_camera *c, const uint8_t *key, double dt);
...
void			triangulate(t_triangle *trs,
					int *trs_size, t_polygon *polygon);
...
double			gradient_calc_x_step(double coords[3],
								t_triangle tr, double one_over_dx);
double			gradient_calc_y_step(double coords[3],
								t_triangle tr, double one_over_dy);
...
void			draw_line(t_app *app,
						t_v3d *start, t_v3d *end, uint32_t color);
...
t_mat4x4		matrix_persp(double fov,
						double ar, double z_near, double z_far);
