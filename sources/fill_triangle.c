...
void	sort_triangle_by_y(t_triangle *in, t_triangle *out)
{
	...
	set_vertex(out->v[0], in->v[0]->x, in->v[0]->y, in->v[0]->z);
	set_vertex(out->v[1], in->v[1]->x, in->v[1]->y, in->v[1]->z);
	set_vertex(out->v[2], in->v[2]->x, in->v[2]->y, in->v[2]->z);
	if (out->v[1]->y < out->v[0]->y)
		SWAP(out->v[0], out->v[1], t_vertex *);
	if (out->v[2]->y < out->v[1]->y)
		SWAP(out->v[1], out->v[2], t_vertex *);
	if (out->v[1]->y < out->v[0]->y)
		SWAP(out->v[0], out->v[1], t_vertex *);
}
