...
int		wall_check_collision(t_wall *w, t_v3d *pos, t_v3d *f)
{
	...
	collide = is_colliding(vector_sum(*pos, *f), 0.15, w->v[2], w->v[0]);
	if (collide)
	{
		normal = get_triangle_normal(w->v[1], w->v[2], w->v[0]);
		normal = vector_mul_by(normal, vector_dot_product(*f, normal));
		*f = vector_sub(*f, normal);
		*pos = vector_sub(*pos, normal);
	}
	return (collide);
}
