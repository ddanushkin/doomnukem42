...
int		event_handling(t_app *app)
{
	...
	app->camera->rotation = matrix_rotation(app->camera->rot.x, app->camera->rot.y, 0);
	app->camera->dir = matrix_multiply_vector(app->camera->rotation, new_vector(0.0, 0.0, 1.0));
	app->camera->target = vector_sum(app->camera->pos, app->camera->dir);
	app->camera->view = matrix_look_at(app->camera->pos, app->camera->target);
	...
	app->camera->view_projection = matrix_multiply(app->camera->projection, matrix_multiply(app->camera->view, app->camera->translation));
	return (1);
}